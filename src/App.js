...
function App() {
  ...
  const sortProducts=(selectedSort)=>{
    if (selectedSort === 'asc') {
      const sorted = [...filterData].sort((a, b) => a.title.localeCompare(b.title));
      setfilterData(sorted);
    } else if (selectedSort === 'desc') {
      const sorted = [...filterData].sort((a, b) => b.title.localeCompare(a.title));
      setfilterData(sorted);
    } else if (selectedSort === 'hl') {
      const sorted = [...filterData].sort((a, b) => b.price - a.price);
      setfilterData(sorted);
    } else if (selectedSort === 'lh') {
      const sorted = [...filterData].sort((a, b) => a.price - b.price);
      setfilterData(sorted);
    }
  ...
  return (
    <div className='container-fluid m-0 p-0'>
      <Navbar count = {count} onSearch={onSearch} />
        <div className='d-flex'>
       <div className='col-lg-2'>
        <Categories onCategorySelect = {onCategorySelect} /><div>
           <Select onSort={sortProducts} /></div>
           <div> <div className='mt-4'><FormButton/></div>
    <div className='mt-4'><ModalButton/></div></div>
        </div>
        
       
 {filterData.length > 0 ? (
        <ProductList display={filterData} addTocart = {addTocart} />
      ) : (
        <p>Loading...</p>
      )}
...
//   };
// const onCategorySelect =(onCategorySelect)=>{
